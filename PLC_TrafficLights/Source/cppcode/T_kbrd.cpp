...
#ifdef CREATE_EVENTFILE

#include "..\..\Class\SigCLib\SigCLib.h"
static _uint32 Taxo = 0;
static void AddWatch_OSEVENT(_OSEVENT *p, const char *title)
{
  char txt[128], crlf[3];
  ...
  void *hdl = sigclib_cfopen("c:\\Event1.txt", 'a');
  
  sigclib_cfputs(hdl, title); sigclib_cfputs(hdl, crlf);
 
  if(Taxo == 0) Taxo = sigclib_tabsolute();
  sigclib_sprintf(txt, "_OSEVENT: Time: %d", sigclib_tabsolute() - Taxo);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
 
  sigclib_sprintf(txt, "eventTime %d", p->eventTime);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventType %d", p->eventType);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "eventSource %d", p->eventSource);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventChar %d", p->eventChar);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventScan %d", p->eventScan);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventState %d", p->eventState);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventButtons %d", p->eventButtons);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventX %d, eventY %d", p->eventX, p->eventY);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventUser[0] %d", p->eventUser[0]);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventUser[1] %d", p->eventUser[1]);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "eventSourceID %d", p->eventSourceID);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_cfputs(hdl, crlf);  
  
  sigclib_cfclose(hdl);
}

static void AddWatch_EVENT(_EVENT *p, const char *title)
{
  char txt[128], crlf[3];
  ...
  if(Taxo == 0) Taxo = sigclib_tabsolute();
  sigclib_sprintf(txt, "_EVENT: Time: %d", sigclib_tabsolute() - Taxo);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
 
  sigclib_sprintf(txt, "ftype %d", p->ftype);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
  
  sigclib_sprintf(txt, "scancode %d", p->scancode);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "modifier %d", p->modifier);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "dot.x %d, dot.y %d", p->dot.x, p->dot.y);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "button %d", p->button);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "user[0] %d", p->user[0]);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "user[1] %d", p->user[1]);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "selfmade %d", p->selfmade);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "source %d", p->source);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "state %d", p->state);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "is_character %d", p->is_character);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "raw.x %d, raw.y %d", p->raw.y, p->raw.y);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "lrm %s", (p->lrm == _TRUE)? "true":"false");
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "timestamp %d", p->timestamp);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "multindex %d", p->multindex);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);

  sigclib_sprintf(txt, "already_used %d", p->already_used);
  sigclib_cfputs(hdl, txt); sigclib_cfputs(hdl, crlf);
...
_BOOL _Keyboard::AddEventEx(_EVENT *evnt, _BOOL keep_timestamp)
{
  ...
  osevent.eventSourceID = (evnt->lrm == _FALSE)? 0xFFFFL : 0xFFFEL;
...
//   #endif
    ...
    case _EVENT_LASALOS    : osevent.eventType    = event_LASALOS; // SA40475
                             ...
                             osevent.eventUser[0] = evnt->user[0];
                             osevent.eventUser[1] = evnt->user[1];
                             osevent.eventButtons = evnt->button;
                             osevent.eventScan    = evnt->scancode;
                             osevent.eventSource  = evnt->source;
                             osevent.eventChar    = evnt->modifier;
                             osevent.eventState   = evnt->state;
                             break;
...
void _Keyboard::DoOsEvent(_OSEVENT *p)
{
  ...
  switch(p->eventSource)
  {
    ...
    default :           Event.ftype    = _EVENT_LASALOS;
                        ...
                        Event.user[0]  = p->eventUser[0];
                        Event.user[1]  = p->eventUser[1];
...
_BOOL _Keyboard::DoAbsolutKeyboard(void)
{
  ...
  if(doit == _IDLE)
  {
    if(RTOSVERSION >= 0x1164)
    {
      ...
      {
       #ifdef CREATE_EVENTFILE
        AddWatch_OSEVENT(&osevent, "OSEVENT from queue");
       #endif
        doit = _IDIDIT;
      }
    }
